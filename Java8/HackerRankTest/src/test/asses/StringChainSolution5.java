...
public class StringChainSolution5 {
    ...
    static int longestChain(List<String> words) {
    	...
    	for (java.util.Map.Entry<Integer, List<String>> entry : charArraysEntriesList) {
    		findChain(charArraysLinkedMap, maxLen, maxChainsCount, entry);
		}
	...
	private static void findChain(LinkedHashMap<Integer, List<String>> charArraysLinkedMap, int maxLen,
		...
		for (String chars : charArraysList) {
			...
			findNextInChain(chain, chars, charArraysLinkedMap, maxChainsCount, maxLen);
	...
	private static void findNextInChain(ArrayList<String> chain, String chars, HashMap<Integer, List<String>> charArraysLinkedMap, java.util.concurrent.atomic.AtomicInteger maxChainsCount, int maxLen) {
		...
		if (!charArraysLinkedMap.containsKey(charsRef.length() + 1)) {
			...
			setMaxChainsCount(maxChainsCount, chain);
		} else {
	...
	private static void setMaxChainsCount(java.util.concurrent.atomic.AtomicInteger maxChainsCount, ArrayList<String> chain) {
