...
public class StringChainSolution {
    ...
    static int longestChain(List<String> words) {
    	...
    	Map<Integer, List<String>> charArraysLinkedMap = wordsRef.parallelStream().collect(Collectors.groupingBy(str -> str.length(), () -> new LinkedHashMap<>(), Collectors.toList()));
    	...
    	charArraysLinkedMap.entrySet().parallelStream().forEach(entry -> findChain(charArraysLinkedMap, maxLen, maxChainsCount, entry));
    	
    	return maxChainsCount.get();

    }

	private static void findChain(Map<Integer, List<String>> charArraysLinkedMap, int maxLen,
	...
	private static void findNextInChain(ArrayList<String> chain, String chars, Map<Integer, List<String>> charArraysLinkedMap, java.util.concurrent.atomic.AtomicInteger maxChainsCount, int maxLen) {
